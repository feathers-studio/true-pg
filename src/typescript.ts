...
import type { createGenerator, Generator, GeneratorMode } from "./types";

export const TypeScript: createGenerator = (mode: GeneratorMode, opts = { enumTo: "enum" }): Generator => {
	...
	return {
		...
		table(types, table) {
			...
			if (mode === "insert") ifaceName = table.name + "_insertable";
			if (mode === "update") ifaceName = table.name + "_updatable";

			if (table.comment && mode === "absolute") out += `/** ${table.comment} */\n`;
